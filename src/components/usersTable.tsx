...
// Removemos o UserDetailsModal, pois os detalhes serão exibidos inline
...
const UsersTable = ({ searchFilter, showFilters, setShowFilters, setSearchFilter }: UsersTableProps) => {
  const [selectedUser, setSelectedUser] = useState<any>(null);
  const [userToDelete, setUserToDelete] = useState<any>(null);
  const { data: utilizadoresInfo, isLoading } = useQuery(getUtilizadoresInfo);
  ...
  const handleDelete = async () => {
    try {
      await updateUserEstadoMutation({ id: userToDelete.id, EstadoUtilizador: userToDelete.EstadoUtilizador });
              ...
              <div key={utilizador.id}>
                ...
                <div className="grid grid-cols-12 border-t border-stroke py-4.5 px-4 dark:border-strokedark md:px-6">
                  ...
                  <div className="col-span-2 flex items-center justify-center gap-2">
                    ...
                    <button
                      title="Eliminar"
                      onClick={() => setUserToDelete(utilizador)}
                ...
                {isOpen && (
                  <div className="grid grid-cols-12 border-t border-stroke py-4 px-4 bg-gray-50">
                    <div className="col-span-12">
                      ...
                      <p className="text-sm text-black dark:text-white">
                        Contacto: {contacto?.Telemovel || 'Não disponível'}
                      </p>
                      <p className="text-sm text-black dark:text-white">
                        Número de subscrições: {subscricoes?.length || 0}
                      </p>
                      {/* Insira aqui outros detalhes que desejar */}
                    </div>
                  </div>
                )}
              </div>
