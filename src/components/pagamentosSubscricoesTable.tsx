...
import { getPagamentosConcluidos } from 'wasp/client/operations';
import LoadingSpinner from '../layout/LoadingSpinner';

const PagamentosSubscricoesTable = () => {
  const [tempSearch, setTempSearch] = useState('');
  const [searchFilter, setSearchFilter] = useState<string | undefined>(undefined);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);

  const { data: response, isLoading } = useQuery(getPagamentosConcluidos, {
    page: currentPage,
    pageSize,
    ...
    <div className="w-full transition-all duration-300">
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="grid grid-cols-12 border-t-4 border-stroke py-4.5 px-4 dark:border-strokedark md:px-6">
          <div className="col-span-3 font-medium">Nome do Utilizador</div>
          <div className="col-span-2 font-medium">NIF</div>
          <div className="col-span-3 font-medium">Plano Subscrito</div>
          <div className="col-span-2 font-medium">Duração</div>
          <div className="col-span-2 font-medium">Valor Pago</div>
        </div>
        ...
        {isLoading ? (
          ...
          pagamentos.map(({ pagamento, utilizador }, i) => {
            const pagamentoComSubs = pagamento as any;
            const subscricao = pagamentoComSubs.Subscricoes[0];
            const tipoPlano = subscricao?.TipoSubscricao?.Nome || '–';
            const duracao = subscricao?.Duracao?.Nome || '–';
            const valorFinal = subscricao?.Detalhes?.[0]?.ValorFinal || 0;
...
export default PagamentosSubscricoesTable;
